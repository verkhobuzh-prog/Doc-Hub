...
export function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    const selectedDocIds = useScopeStore.getState().selectedDocIds
    const scopedIds = selectedDocIds.filter((id) =>
      indexedDocs.some((d) => d.id === id),
    )

    const request: ChatRequest = { query }
    if (scopedIds.length > 0) {
      request.context_doc_ids = scopedIds
    }
